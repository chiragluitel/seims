...
interface WebsiteProductCardPreviewProps {
    product: Product
    onClick: (product: Product) => void;
}

const WebsiteProductCardPreview: React.FC<WebsiteProductCardPreviewProps> = ({ product, onClick }) => {
    return (
        <div
                ...
                <div className="flex items-start justify-between gap-3">
                    ...
                    {product.online_price_cents !== undefined && (
                        <span className="shrink-0 rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700 border border-emerald-100">
                            ${(product.online_price_cents/100).toFixed(2)}
                        </span>
                    )}
                </div>
...
export default WebsiteProductCardPreview;

...
const NewProductForm: React.FC<NewProductFormProps> = ({ onInputChange, onSubmit, onNestedInputChange }) => {
  ...
  const {uploadFunction, s3ImageUrlResponse} = useS3UploadImage();
  const OnUpload = (file: File | null ) => {
      if(!file){
        return
      }
      uploadFunction(file);
      if(s3ImageUrlResponse){
        onInputChange('image', s3ImageUrlResponse)
      }   
  }
  return (
    ...
    <NumberInputBox label="In-Store Price ($)*" id="product-price-instore" placeholder="e.g., 15.00" onInputChange={onInputChange} labelhtmlfor="product-price-instore" fieldof="instore_price" />
    <NumberInputBox label="Online Price ($)*" id="product-price-online" placeholder="e.g., 15.00" onInputChange={onInputChange} labelhtmlfor="product-price-online" fieldof="online_price"/>
    ...
    <NumberInputBox label="Current Stock" id="product-soh" placeholder="e.g., 15" onInputChange={onInputChange} labelhtmlfor="product-soh" fieldof="soh"/>

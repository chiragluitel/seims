    ...
    <div className="flex flex-col">
        ...
        <div className="relative">
          <input
            ...
            onChange={
              (e) => {onUpload(e.target.files? e.target.files[0]:null); setFileName(e.target.files? e.target.files[0].name:null) }
            }
          />
          <div className="flex items-center justify-center p-3 rounded-lg border-2 border-dashed border-gray-600 cursor-pointer hover:bg-gray-700 transition-colors">
            ...
            {fileName? (<span className="text-gray-400">  {fileName} </span>) : (<span className="text-gray-400">Click to upload image</span>)}
          </div>
        </div>
    </div>
